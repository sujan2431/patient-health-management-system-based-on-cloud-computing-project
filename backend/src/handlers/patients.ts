...
export async function handler(event: ApiEvent): Promise<ApiResponse> {
  ...
  if (event.httpMethod === "POST" && event.path === "/patients") {
    ...
    await logAction({
      ...
      resource_id: created.patient_id.toString(),
  ...
  if (event.httpMethod === "GET" && event.path.startsWith("/patients/")) {
    ...
    if (!Number.isFinite(id) || id <= 0) return badRequest("Invalid patient id");
    ...
    if (authed.user.role !== "Doctor" && authed.user.role !== "Admin") return notFound("Patient not found");

    await logAction({
      user_id: authed.user.sub,
      action: "VIEW_PATIENT",
      resource_type: "Patient",
      resource_id: (patient as { patient_id: number }).patient_id.toString()
    });
