...
export async function handler(event: ApiEvent): Promise<ApiResponse> {
  ...
  if (event.httpMethod === "POST" && event.path === "/reports/presign-upload") {
    ...
    await logAction({
      user_id: authed.user.sub,
      action: "UPLOAD_REPORT",
      resource_type: "Report",
      resource_id: created.report_id.toString(),
      details: { patient_id: parsed.data.patient_id, filename: parsed.data.filename }
    });
  ...
  if (event.httpMethod === "GET" && event.path === "/reports") {
    ...
    if (!patient) return ok({ reports: [] });
    const p = patient as { patient_id?: number };
  ...
  if (event.httpMethod === "GET" && event.path.startsWith("/reports/") && event.path.endsWith("/download")) {
    ...
    if (!Number.isFinite(reportId) || reportId <= 0) return badRequest("Invalid report id");

    const report = await getReportById(reportId);
    if (!report) return notFound("Report not found");

    if (authed.user.role === "Doctor") {
      if (report.uploaded_by_doctor_id !== authed.user.sub) return notFound("Report not found");
    } else if (authed.user.role === "Patient") {
      const patient = await getPatientByUserId(authed.user.sub);
      const p = patient as { patient_id?: number } | null;
      if (!p?.patient_id || Number(p.patient_id) !== report.patient_id) return notFound("Report not found");
    } // Admin allowed
    ...
    await logAction({
      ...
      resource_type: "Report",
      resource_id: report.report_id.toString(),
